...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        N  = x.shape[0]
        d1 = x.shape[1]
        dx = torch.matmul(dout,torch.t(w)).reshape(N,x.shape[1],x.shape[2],-1).squeeze()
        dw = torch.matmul(torch.t(x.reshape(N, -1)),dout)
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        out_linear, _ = Linear.forward(x, w, b)
        out, _ = ReLU.forward(out_linear)
        fc_cache = (x,w,b)
        relu_cache = out_linear
    ...
    def backward(dout, cache):
        ...
        dout_relu = ReLU.backward(dout, relu_cache)
        dx, dw, db = Linear.backward(dout_relu, fc_cache)
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    for i in range(y.shape[0]):
        exp_values = torch.exp(x[i])
        exp_values_sum = torch.sum(exp_values)
        softmax_function_output = exp_values/exp_values_sum
        y_preds = softmax_function_output
        loss += -1 *torch.log( torch.max(y_preds) )
